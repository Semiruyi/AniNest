...
```powershell
dotnet build LocalPlayer.sln
dotnet run --project .\src\LocalPlayer\LocalPlayer.csproj
```

发布：

```powershell
dotnet publish .\src\LocalPlayer\LocalPlayer.csproj -c Release -r win-x64 --self-contained true -o .\artifacts\publish
```
...
```powershell
dotnet test .\src\Tests\LocalPlayer.Tests.csproj
```
